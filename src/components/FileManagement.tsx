...
const FileManagement: React.FC<FileManagementProps> = ({ files, dealerId, dealerName }) => {
  ...
  const [fileStates, setFileStates] = useState<FileSetting[]>([]);
  ...
  useEffect(() => {
    if (dealer?.fileSettings) {
      setFileStates(dealer.fileSettings);
    }
  }, [dealer?.fileSettings]);
  ...
  const getFileIcon = (file: FileSetting) => {
    if (!file.enabled) {
  ...
  const getStatusColor = (file: FileSetting) => {
    if (!file.enabled) return 'bg-gray-100 text-gray-500 border-gray-200';
  ...
  const toggleFileEnabled = async (fileName: string) => {
    ...
    setFileStates(prev => prev.map(file => {
      if (file.name === fileName) {
        ...
        console.log(`Toggling ${file.status} to ${newEnabled}`);
        return {
          ...file,
          enabled: newEnabled,
        };
    ...
    toast({
      title: `${fileName} ${file?.enabled ? false : true}`,
      description: `File ${fileName} has been ${file?.enabled ? false : true} for ${dealerName}`,
    });
  };
  ...
  const sendFile = async (fileName: string) => {
    ...
    setFileStates(prev => prev.map(file => {
      if (file.name === fileName) {
        return {
          ...file,
          status: isSuccess ? file.status : 'error',
          ...
          {fileStates.map((file, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <Badge className={`text-xs ${getStatusColor(file)}`}>
                  {file.enabled ? file.status : 'disabled'}
                </Badge>
